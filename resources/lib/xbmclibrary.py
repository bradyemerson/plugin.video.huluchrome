...
if (common.settings['enablelibraryfolder'] == 'true'):
    MOVIE_PATH = os.path.join(xbmc.translatePath('special://profile/addon_data/%s/' % common.addonName),'Movies')
    TV_SHOWS_PATH = os.path.join(xbmc.translatePath('special://profile/addon_data/%s/' % common.addonName),'TV')
...
class Main:
    ...
    def ExportShowMovie(self, item):
        ...
        if feature_films_count == '1' or feature_films_count == 1:
            ...
            strm = 'plugin://%s/?mode="TV_play"' % (common.addonName)
